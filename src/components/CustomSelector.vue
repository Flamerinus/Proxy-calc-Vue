...
function copyText() {
  ...
  const infoText =
    ...
    "Select all the options and info text will show";
...
onMounted(() => {
  // Code to run after everything is mounted
  const input = document.getElementById("NumericInputId");
...
<template>
  ...
  <div class="mx-auto my-1">
    ...
    <Input id="NumericInputId" type="number" placeholder="Please write your bitrate (1-99999)" min="1" max="99999"
      v-model="bitRate" />
  </div>
  <div class="mx-auto my-1">
    ...
    <TooltipProvider v-if="selectedDuration && bitRate && mega">
      <Tooltip>
        <TooltipTrigger class="flex flex-auto justify-center">
          <Badge class="flex-1 justify-center" variant="secondary">File size: {{ formattedFileSize }}</Badge>
        </TooltipTrigger>
        <TooltipContent>
    ...
    <Alert v-if="selectedDuration && bitRate && mega">
      <AlertDescription>
        <span id="info">
          ...
          {{ bitRate }} {{ unit }}/s will take up approximately
          {{ formattedFileSize }} of disk space.
          (flamerinus.github.io/Proxycalc)
        </span>
      </AlertDescription>
    </Alert>
